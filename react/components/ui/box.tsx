...
export interface BoxProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  borderColor?: ResponsiveValue<VariantProps<typeof boxVariants>['borderColor']>;
...
export const Box = memo(forwardRef<HTMLDivElement, BoxProps>(
  ({
    ...
    if (gridTemplateColumns) gridProps.gridTemplateColumns = gridTemplateColumns;
    ...
    Object.entries(responsiveProps).forEach(([breakpoint, breakpointProps]) => {
      ...
      try {
        ...
        if (breakpointClasses) {
          (Array.isArray(breakpointClasses) ? breakpointClasses : [breakpointClasses])
            .filter(Boolean)
            .forEach(cls => {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.warn(`Error processing responsive props for breakpoint ${breakpoint}:`, err);
        }
      }
    });
 ...
 */
export const Container = memo(forwardRef<HTMLDivElement, Omit<BoxProps, 'mx' | 'maxWidth'>>(
  (props, ref) => (
    <Box
      ref={ref}
      mx="auto"
 ...
 */
export const Flex = memo(forwardRef<HTMLDivElement, Omit<BoxProps, 'display'>>(
  (props, ref) => (
 ...
 */
export const Grid = memo(forwardRef<HTMLDivElement, Omit<BoxProps, 'display'>>(
  (props, ref) => (
 ...
 */
export const Card = memo(forwardRef<HTMLDivElement, BoxProps>(
  (props, ref) => (
    <Box
      ref={ref}
      p={5}
 ...
 */
export const Divider = memo(forwardRef<HTMLDivElement, BoxProps & { orientation?: 'horizontal' | 'vertical' }>(

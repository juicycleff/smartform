...
interface EnhancedSelectProps<T extends string = string>
  extends Omit<React.ComponentPropsWithoutRef<typeof Button>, 'color' | 'variant' | 'onChange' | 'size'>,
  ...
  (
    ...
    const filteredOptions = React.useMemo(() => {
      ...
      return options.filter(
        (option) =>
          String(option[optionLabel]).toLowerCase().includes(search.toLowerCase()) ||
          String(option[optionValue]).toLowerCase().includes(search.toLowerCase()),
      )
    }, [options, search, searchable, optionLabel, optionValue])

    const selectedLabels = React.useMemo(() => {
      return selectedValues
        .map((v) => options.find((option) => option[optionValue] === v)?.[optionLabel])
    ...
    const singleSelect = (
      ...
      >
        <SelectTrigger
          id={id}
          className={cn(selectVariants({ radius }), error && "border-destructive", triggerClassName)}
        >
          <SelectValue placeholder={<span className="placeholder:text-left placeholder:truncate placeholder:text-ellipsis">placeholder</span>} />
        </SelectTrigger>
        <SelectContent
              ...
              {renderOption ? (
                ...
                <div className="flex flex-col">
                  ...
                  {option[optionDescription] && (
                    <span className="text-xs text-muted-foreground">{option[optionDescription]}</span>
                  )}
                </div>
              )}
            </SelectItem>
      ...
      if (label) {
        return (
          <div className="space-y-2">
            <Label htmlFor={id} className={cn("text-pyro-text-primary", labelClassName)}>
              {label}
            </Label>
            {singleSelect}
          </div>
